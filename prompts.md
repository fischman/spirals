...
### `2026-03-04 12:26 PST`
```
In hilbert order 7, the curve is rendered (correct). In order 8 and higher, the curve disappears (which is fine, it's already too dense to be usefully shown) but also the canvas background flips to a solid lighter grey making it harder to see the points of interest, e.g.  on https://fischman.github.io/spirals/#hilbert,squares,11

Figure out why this switch, and stop doing it.
(other spiral types don't have this problem)
```

### `2026-03-04 12:30 PST`
```
can you avoid busybox/httpd/caching concerns by always loading from a file:// URL?
```
